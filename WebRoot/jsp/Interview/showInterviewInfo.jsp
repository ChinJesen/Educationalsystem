...
<script type="text/javascript">
   ...
   function _delete(ivId)
   {
	   if(window.confirm("确认删除？"))
		{
		   window.location.href="<%=path%>/ivMrg?flag=delete&id="+ivId; 	
		}
   }
   function _modify(ivId)
   {
	   window.location.href="<%=path%>/ivMrg?flag=queryById&id="+ivId;   
   }
</script>
<script type="text/javascript">
//通过班级获取班级内所有学生信息
...
<body>
	...
	<fieldset>
		...
		<table border="1" width="70%" align="center">
			...
			<c:forEach items="${pageUtil.list}" var="ivInfo" varStatus="_v">
				<tr>
					<td>${_v.count}</td>
					<td>
			        <c:forEach items="${classMap}" var="c">
  			        <c:if test="${c.key eq ivInfo.classId}">
  			          ${c.value}
  			        </c:if>
  			        </c:forEach>
			        </td>
			        ...
			        <c:if test="${c.key eq ivInfo.stuId}">
			          ${c.value}
			        </c:if>
			        </c:forEach>
			        </td>
					<td>${ivInfo.ivSex}</td>
					<td>${ivInfo.ivTime}</td>
					<td>${ivInfo.ivInformation}</td>
					<td>${ivInfo.ivFollow}</td>
					<td>${ivInfo.document}</td>
					<td>${ivInfo.remark}</td>
						
					<td align="center">
						<input type="button" value="删除" onclick="_delete('${ivInfo.ivId}')"/>
						<input type="button" value="修改" onclick="_modify('${ivInfo.ivId}')"/>
					</td>
				</tr>
			</c:forEach>
		</table>		
