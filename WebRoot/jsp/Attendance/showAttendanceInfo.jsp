...
<script type="text/javascript">
   function _modify(adId)
   {
	   window.location.href="<%=path%>/adMrg?flag=queryById&id="+adId;   
   }
   function _delete(adId)
   {
	   if(window.confirm("确认删除？"))
		{
		   window.location.href="<%=path%>/adMrg?flag=delete&id="+adId; 	
		}
   }
</script>
<script type="text/javascript">
...
<body>
	...
	<fieldset>
		...
		<table border="1" width="70%" align="center">
			...
			<c:forEach items="${pageUtil.list}" var="adInfo" varStatus="_v">
				<tr>
					<td>${_v.count}</td>
					<td>
			   <c:forEach items="${classMap}" var="c">
  			       <c:if test="${c.key eq adInfo.classId}">
  			          ${c.value}
  			       </c:if>
  			       </c:forEach>
			</td>
			<td>
				<c:forEach items="${stuNameMap}" var="c">
			       <c:if test="${c.key eq adInfo.stuId}">
			          ${c.value}
			       </c:if>
			       </c:forEach>
			</td>
					<td>${adInfo.adTime}</td>
					<td>${adInfo.adsituation}</td>
					<td>${adInfo.remark}</td>
					<td align="center">
						<input type="button" value="删除" onclick="_delete('${adInfo.adId}')"/>
						<input type="button" value="修改" onclick="_modify('${adInfo.adId}')"/>
					</td>
				</tr>
			</c:forEach>
		</table>		
